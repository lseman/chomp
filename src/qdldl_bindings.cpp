...
static inline SparseD64 make_sparse_from_csc(
    ...
    return SparseD64(static_cast<std::int64_t>(n), std::move(Ap), std::move(Ai),
                     std::move(Ax));
}

// Build Ordering from a user-supplied permutation vector (no built-ins)
static inline Ordering<std::int64_t> make_ordering_from_perm(
    ...
    for (std::int64_t i = 0; i < n; ++i) {
        ...
        if (pi < 0 || pi >= n)
            throw std::invalid_argument("perm contains out-of-range index");
        if (seen[static_cast<std::size_t>(pi)])
            throw std::invalid_argument(
                "perm is not a permutation (duplicates)");
...
static inline bool parse_external_perm(py::object perm_obj, std::int64_t n,
        ...
        if (parent[(size_t)r] == -1) {
            ...
            while (!st.empty()) {
                ...
                if (!entered[(size_t)u]) {
                    ...
                    // push children in reverse sibling order to visit
                    // first_child first
                    std::vector<IntT> kids;
                    for (IntT c = first_child[(size_t)u]; c != -1;
                         c = next_sib[(size_t)c])
                        kids.push_back(c);
...
static inline void build_permuted_structure_and_map(
    ...
    for (std::int64_t j = 0; j < n; ++j) {
        const auto pj = ord ? ord->perm[(size_t)j] : j;
        for (auto p = A.Ap[(size_t)j]; p < A.Ap[(size_t)j + 1];
             ++p, ++src_idx) {
...
PYBIND11_MODULE(qdldl_cpp, m) {
    m.doc() =
        "QDLDL-style LDL^T (header-only, C++23) Python bindings with "
        "external-only permutation and cached fast refactorization";
    ...
    py::class_<SymbolicHandle>(m, "Symbolic")
        ...
        .def_property_readonly("L_indptr",
                               [](const SymbolicHandle& s) {
                                   return py::array(s.S.Lp.size(),
                                                    s.S.Lp.data());
                               })
        .def_property_readonly("Lnz",
                               [](const SymbolicHandle& s) {
                                   return py::array(s.S.Lnz.size(),
                                                    s.S.Lnz.data());
                               })
        .def_property_readonly("Bp",
                               [](const SymbolicHandle& s) {
                                   return py::array(s.Bp.size(), s.Bp.data());
                               })
        .def_property_readonly("Bi",
                               [](const SymbolicHandle& s) {
                                   return py::array(s.Bi.size(), s.Bi.data());
                               })
        .def_property_readonly("A2B",
                               [](const SymbolicHandle& s) {
                                   return py::array(s.A2B.size(), s.A2B.data());
                               })
        .def("__repr__", [](const SymbolicHandle& s) {
            return "<qdldl_cpp.Symbolic n=" + std::to_string(s.n) +
                   (s.has_perm ? " (with external perm; cached structure)>"
                               : " (cached structure)>");
        });
    ...
    m.def(
        ...
        [](py::array_t<std::int64_t> indptr, py::array_t<std::int64_t> indices,
           ...
           py::object perm /* None | np.array[int64] */) {
            ...
            // Build cached structure + map (B is structure only here; values
            // ignored)
            build_permuted_structure_and_map(A, use_perm ? &ord : nullptr,
                                             out.Bp, out.Bi, out.A2B);

            // Analyze on the cached structure by creating a temporary Sparse
            // with zeros
            {
            ...
            etree_orders(out.S.etree, out.preorder, out.postorder,
                         out.first_child, out.next_sib);
    ...
    m.def(
        ...
        [](py::array_t<std::int64_t> indptr, py::array_t<std::int64_t> indices,
           ...
           py::object perm /* None | np.array[int64] */) {
            ...
            LDL64 fac =
                (!use_perm) ? factorize(A) : factorize_with_ordering(A, ord);
    ...
    m.def("factorize_scipy", [](py::array_t<std::int64_t> indptr,
    ...
    m.def(
        "refactorize",
        [](const SymbolicHandle& S, py::array_t<std::int64_t> indptr,
           py::array_t<std::int64_t> indices, py::array_t<double> data,
           std::int64_t n) {
            if (n != S.n)
    ...
    m.def(
        ...
        "Solve A x = b using an existing Factor (uses external permutation if "
        "present).");
    ...
    m.def(
        "solve_refine",
        [](const Factor& F, py::array_t<std::int64_t> indptr,
           py::array_t<std::int64_t> indices, py::array_t<double> data,
           py::array_t<double> b, int iters) {
            if (b.ndim() != 1) throw std::invalid_argument("b must be 1-D");
            if (b.shape(0) != F.n)
                throw std::invalid_argument("b length must equal n");
            ...
            if (!F.has_ordering) {
                ...
                qdldl23::refine<double, std::int64_t>(
                    ...
                    static_cast<const qdldl23::Ordering<std::int64_t>*>(
                        nullptr));
            } else {
                Ordering<std::int64_t> ord;
                ord.n = F.n;
                ...
                ord.iperm = F.iperm;
                solve_with_ordering(fac, ord, x);
                qdldl23::refine<double, std::int64_t>(A, fac, x, b.data(),
                                                      iters, &ord);
            }
            return out;
        },
        ...
        "Solve with iterative refinement against the original A (uses external "
        "permutation if present).");

    m.def("version", []() {
        return std::string(
            "qdldl_cpp 3.0 — cached-permutation fast refactorization");
    });
}
