...
NB_MODULE(ad, m) {
    ...
    nb::class_<Variable>(m, "Variable")
        .def(
            nb::init<const std::string&, double, double, double>(),
            "name"_a,
            "value"_a = 0.0,
        ...
        .def("__rpow__", [](const std::shared_ptr<Variable>& v, double s) {
            auto g  = std::make_shared<ADGraph>();
            auto ex = std::make_shared<Expression>(v, 1.0, g);
            // Use generic pow dispatch (handles domain checks & fallback)
            return pow_dispatch(nb::float_(s), nb::cast(ex));
        });
    ...
    nb::class_<Expression>(m, "Expression")
        ...
        // scalar ⊕ Expression (reverse ops)
        .def("__radd__",     [](const Expression& a, double s) { return s + a; })
        .def("__rsub__",     [](const Expression& a, double s) { return s - a; })
        .def("__rmul__",     [](const Expression& a, double s) { return s * a; })
        ...
        .def("__pow__", [](const std::shared_ptr<Expression>&,
                           const std::shared_ptr<Expression>&) {
            throw std::invalid_argument(
                "Expression ** Expression not supported; exponent must be scalar.");
        })
        .def("__rpow__", [](const std::shared_ptr<Expression>& a, double s) {
            return pow_dispatch(nb::float_(s), nb::cast(a));
        });

    // ---------------- Immediate APIs ----------------
    m.def("valgrad",
          [](nb::object f, Arr1D x) { return py_value_grad_numpy(f, x); },
          "f"_a, "x"_a,
    ...
    m.def("grad",
          [](nb::object f, Arr1D x) { return py_gradient_numpy(f, x); },
          "f"_a, "x"_a,
          "Gradient at x (ndarray[float64], cached).");

    // ---------------- Unary registrations (scalar ops) ----------------
    auto reg_unary = [&](const char* name, Operator op, const char* doc) {
        m.def(name, [op](nb::object x) { return unary_dispatch(std::move(x), op); }, doc);
    };
    ...
    // NOTE: softmax omitted here (vector-valued op). Expose a dedicated API if needed.

    // ---------------- max / pow helpers ----------------
    m.def("max",
          [](nb::object a, nb::object b) { return nb::cast(max_dispatch(a, b)); },
          "max(a,b) with subgradient 0.5/0.5 at ties");

    m.def("pow",
          [](nb::object x, double p) {
              // Expression ** scalar OR scalar ** scalar -> handled; scalar ** Expression: use pow_dispatch
              if (is_number(x)) {
                  // returns an Expression constant for consistency
                  return nb::cast(coerce_expr(nb::float_(std::pow(nb::cast<double>(x), p))));
              }
              auto ex = coerce_expr(x);
              return nb::cast(pow(*ex, p));
          },
          "pow(x,p) builds through Expression; non-integer handled by Expression.");

    // ---------------- Compiled gradient ----------------
    nb::class_<GradFn>(m, "GradFn")
        .def("__call__",      &GradFn::call_numpy,     "x"_a, "Evaluate gradient at x (ndarray)->ndarray")
        .def("__call__",      &GradFn::operator(),     "x_list"_a, "Evaluate gradient at x (list/tuple)->list")
        .def("expr_str",      &GradFn::expr_str)
        .def("value",         &GradFn::value_numpy,    "x"_a, "Evaluate f(x) (ndarray)->float")
        .def("value_grad",    &GradFn::value_grad_numpy, "x"_a, "Return (f(x), grad f(x)) (ndarray)")
    ...
    nb::class_<LagHessFn>(m, "LagHessFn")
        ...
        .def("hvp_multi_numpy", &LagHessFn::hvp_multi_numpy,
             "x"_a, "lam"_a, "nu"_a, "V"_a, "Return H·V (loops over columns; ndarray[n,k])");
