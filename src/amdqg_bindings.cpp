...
static inline std::vector<i32> to_vec_i32(
    const py::array_t<int32_t, py::array::c_style | py::array::forcecast>& a) {
...
static inline py::array_t<i32> to_numpy_i32_from_ptr(const i32* data,
                                                     size_t n) {
...
static inline py::array_t<i32> ranges_to_numpy_2col(
    const std::vector<std::pair<i32, i32>>& ranges) {
...
static CSR make_csr(
    ...
    const py::array_t<i32, py::array::c_style | py::array::forcecast>&
        indices) {
...
static py::dict supernodes_to_pydict(const SupernodeInfo& sn) {
    py::dict d;
    d["ranges"] =
        ranges_to_numpy_2col(sn.ranges);  // shape [ns, 2], inclusive start/end
...
PYBIND11_MODULE(amdqg, m) {
    m.doc() =
        "Array-based AMD with quotient-graph + supernode detection (C++17) — "
        "pybind11 bindings";
    ...
    py::class_<CSR>(m, "CSR")
        ...
        .def("nnz", &CSR::nnz)
        .def("strict_upper_union_transpose",
             [](const CSR& A) { return A.strict_upper_union_transpose(); })
    ...
    m.def("make_csr", &make_csr, py::arg("n"), py::arg("indptr"),
          py::arg("indices"),
          "Create CSR(n, indptr, indices) from int32 NumPy arrays");

    // -------- AMD ----------
    py::class_<AMDReorderingArray>(m, "AMDReorderingArray")
        .def(py::init<bool, int>(), py::arg("aggressive_absorption") = true,
        ...
        .def(
            "compute_fill_reducing_permutation",
            [](AMDReorderingArray& self, const CSR& A, bool symmetrize) {
            ...
            py::arg("A"), py::arg("symmetrize") = true,
            "Return (perm, stats) where perm is int32 array and stats is a "
            "dict")
        // New: convenience that returns (perm, stats, supernodes_dict)
        .def(
            "compute_perm_and_supernodes",
            [](AMDReorderingArray& self, const CSR& A, bool symmetrize,
               int relax, double tau, int max_size) {
                auto pr = self.compute_fill_reducing_permutation(A, symmetrize);
                auto perm_vec = pr.first;
                const AMDStats& st = pr.second;
                SupernodeInfo sn =
                    identify_supernodes(A, perm_vec, relax, tau, max_size);
                py::dict stats;
                ...
                stats["inverse_permutation"] =
                    to_numpy_i32(st.inverse_permutation);
                ...
                return py::make_tuple(to_numpy_i32(perm_vec), stats,
                                      supernodes_to_pydict(sn));
            },
            py::arg("A"), py::arg("symmetrize") = true, py::arg("relax") = 0,
            py::arg("tau") = 1.0,
            py::arg("max_size") = std::numeric_limits<int>::max(),
            "Return (perm, stats, supernodes) with relaxed amalgamation "
            "options");

    // Convenience: pass raw arrays from Python (e.g. SciPy CSR arrays)
    m.def(
        "amd_order_from_csr_arrays",
        [](i32 n,
           const py::array_t<i32, py::array::c_style | py::array::forcecast>&
               indptr,
           const py::array_t<i32, py::array::c_style | py::array::forcecast>&
               indices,
           bool symmetrize, bool aggressive_absorption, int dense_cutoff) {
        ...
        py::arg("symmetrize") = true, py::arg("aggressive_absorption") = true,
        py::arg("dense_cutoff") = -1, "One-shot AMD on (n, indptr, indices)");

    // -------- Supernode identification (free functions) ----------
    // identify_supernodes(A: CSR, p: np.int32[1D], relax=0, tau=1.0,
    // max_size=INT_MAX) -> dict
    m.def(
        "identify_supernodes",
        [](const CSR& A,
           ...
           int relax, double tau, int max_size) {
            ...
            SupernodeInfo sn =
                identify_supernodes(A, pvec, relax, tau, max_size);
            return supernodes_to_pydict(sn);
        },
        py::arg("A"), py::arg("p"), py::arg("relax") = 0, py::arg("tau") = 1.0,
        py::arg("max_size") = std::numeric_limits<int>::max(),
        "Identify structural supernodes on the pattern of A[p,p].\n"
        "Returns dict with:\n"
        "  ranges: int32 array [ns,2] of inclusive column ranges in permuted "
        "space\n"
