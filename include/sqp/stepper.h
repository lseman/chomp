...
// - Uses native Model/regularization/TR logic; Python QP object is optional
...
class SQPStepper {
   public:
    ...
    explicit SQPStepper(nb::object cfg, nb::object qp_solver = nb::none(),
   ...
   private:
    ...
    static TRConfig build_tr_config_from_cfg_(const nb::object& cfg) {
        ...
        // Core TR params
        tc.delta0 = get_attr_or<double>(cfg, "delta0",
                                        get_attr_or<double>(cfg, "tr_delta0", tc.delta0));
        ...
        tc.norm_type = get_attr_or<std::string>(cfg, "norm_type",
                                                get_attr_or<std::string>(cfg, "tr_norm_type", tc.norm_type));
        tc.metric_shift =
    ...
    void init_misc_() {
        requires_dense_ = !qp_.is_none() && nb::hasattr(qp_, "requires_dense")
        ...
        ensure_default("delta0", 1.0);
        ensure_default("tr_delta0", get_attr_or<double>(cfg_, "delta0", 1.0));
        ensure_default("norm_type", std::string("2"));
