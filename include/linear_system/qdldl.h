...
inline void scatter_update(FloatT* __restrict__ x,
                           const IntT* __restrict__ indices,
                           const FloatT* __restrict__ values, IntT n,
                           FloatT scale) {
    ...
    void finalize_upper_inplace(bool require_diag = true) {
        ...
        for (IntT j = 0; j < n; ++j) {
            ...
            for (size_t k = 0; k < col_workspace.size();) QDLDL23_LIKELY {
                    ...
                    while (k2 < col_workspace.size() &&
                           col_workspace[k2].first == r)
                        QDLDL23_UNLIKELY {
...
template <std::floating_point FloatT = double,
          std::signed_integral IntT = int32_t>
inline SparseUpperCSC<FloatT, IntT> permute_symmetric_upper(
    const SparseUpperCSC<FloatT, IntT>& A, const Ordering<IntT>& ord) {
    ...
    for (IntT j = 0; j < n; ++j) {
        ...
        for (IntT p = A.Ap[static_cast<size_t>(j)];
    ...
    for (IntT j = 0; j < n; ++j) {
        ...
        std::stable_sort(indices.begin(), indices.end(), [&](IntT a, IntT b) {
            return Bi[static_cast<size_t>(p0 + a)] <
                   Bi[static_cast<size_t>(p0 + b)];
        });
...
template <std::floating_point FloatT = double,
          std::signed_integral IntT = int32_t>
inline LDLFactors<FloatT, IntT> refactorize(
    const SparseUpperCSC<FloatT, IntT>& A, const Symbolic<IntT>& S) {
    const IntT n = S.n;
    ...
    const size_t aligned_n =
        ((static_cast<size_t>(n) + alignment - 1) / alignment) * alignment;
    ...
    for (IntT k = 1; k < n; ++k) {
        ...
        for (IntT p = p0; p < p1; ++p) {
            ...
            if (!is_marked(i)) QDLDL23_LIKELY {
                    ...
                    while (next != -1 && next < k && nnzE < max_path) {
                        ...
                        const IntT parent_next =
                            S.etree[static_cast<size_t>(next)];
                        if (parent_next >= 0 && parent_next < k &&
                            is_marked(parent_next))
                            break;
                        if (parent_next >= 0 && parent_next < k)
                    ...
                    for (IntT e = nnzE - 1; e >= 0 && nnzY < n; --e) {
                        if (e < static_cast<IntT>(Ebuf.size()) &&
                            ...
                            yIdx[static_cast<size_t>(nnzY++)] =
                                Ebuf[static_cast<size_t>(e)];
                        }
                    }
                }
        }
        ...
        for (IntT t = nnzY - 1; t >= 0; --t) {
            ...
            if (update_len > 0) {
                ...
                if (update_len > 8 && std::is_same_v<FloatT, double>) {
                    detail::scatter_update(
                        y.data(), &R.Li[static_cast<size_t>(j0)],
        ...
        ++R.num_pos;
        R.Dinv[static_cast<size_t>(k)] =
            FloatT{1} / R.D[static_cast<size_t>(k)];
    }
...
template <std::floating_point FloatT = double,
          std::signed_integral IntT = int32_t>
inline LDLFactors<FloatT, IntT> refactorize_with_ordering(
    const SparseUpperCSC<FloatT, IntT>& A, const Symbolic<IntT>& Sperm,
...
template <std::floating_point FloatT = double,
          std::signed_integral IntT = int32_t>
inline LDLFactors<FloatT, IntT> factorize(
    const SparseUpperCSC<FloatT, IntT>& A) {
...
template <std::floating_point FloatT = double,
          std::signed_integral IntT = int32_t>
inline LDLFactors<FloatT, IntT> factorize_with_ordering(
    const SparseUpperCSC<FloatT, IntT>& A, const Ordering<IntT>& ord) {
    ...
    for (IntT i = 0; i < n; ++i) {
        ...
        if (col_len > 8 && std::is_same_v<FloatT, double>) {
            ...
            for (IntT p = p0; p < p1; ++p) {
                ...
                if (idx >= 0 && idx < n) {
                    x[static_cast<size_t>(idx)] -=
                        F.Lx[static_cast<size_t>(p)] * xi;
                }
            }
        }
    }
...
inline void Lt_solve(const LDLFactors<FloatT, IntT>& F, FloatT* x) {
    ...
    for (IntT i = n - 1; i >= 0; --i) {
        ...
        for (IntT p = p0; p < p1; ++p) {
            ...
            if (idx >= 0 && idx < n) {
                xi -=
                    F.Lx[static_cast<size_t>(p)] * x[static_cast<size_t>(idx)];
            }
        }
...
inline void solve_with_ordering(const LDLFactors<FloatT, IntT>& F,
                                const Ordering<IntT>& ord, FloatT* x) {
    const IntT n = F.n;
    ...
    // Diagonal scaling
    if constexpr (std::is_same_v<FloatT, double>) {
        detail::simd_scale_array(xp.data(), F.Dinv.data(),
...
inline void refine(const SparseUpperCSC<FloatT, IntT>& A,
    ...
    for (int it = 0; it < iters; ++it) {
        ...
        if constexpr (std::is_same_v<FloatT, double>) {
            detail::simd_vector_sub(r.data(), b, r.data(),
                                    static_cast<size_t>(n));
        } else {
            for (IntT i = 0; i < n; ++i) {
        ...
        if constexpr (std::is_same_v<FloatT, double>) {
            ...
            detail::qd_par_for_n(static_cast<size_t>(n),
                                 [&](size_t i) { x[i] += t[i]; });
        }
    }
}
