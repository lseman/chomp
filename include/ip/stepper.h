...
// as double.
[[nodiscard]] EIGEN_STRONG_INLINE Sigmas build_sigmas(const dvec &zL, const dvec &zU, const Bounds &B, const dvec &lmb,
                                                      const dvec &s, const dvec &cI, double tau_shift,
                                                      double bound_shift, bool use_shifted, double eps_abs,
                                                      double cap) {
    ...
    if (n > 0) {
        ...
        const Eigen::Map<const Arrb> hasL_u8(reinterpret_cast<const unsigned char *>(B.hasL.data()), n);
        const Eigen::Map<const Arrb> hasU_u8(reinterpret_cast<const unsigned char *>(B.hasU.data()), n);
...
[[nodiscard]] EIGEN_STRONG_INLINE std::pair<dvec, dvec> dz_bounds_from_dx_vec(const dvec &dx, const dvec &zL,
                                                                              const dvec &zU, const Bounds &B,
                                                                              double bound_shift, bool use_shifted,
                                                                              double mu, bool use_mu) {
    const int n   = static_cast<int>(dx.size());
    dvec      dzL = dvec::Zero(n), dzU = dvec::Zero(n);
    if (n == 0) return {dzL, dzU};
    ...
    const ArrXb mL   = (Eigen::Map<const Arrb>(reinterpret_cast<const unsigned char *>(B.hasL.data()), n) != 0);
    const ArrXb mU   = (Eigen::Map<const Arrb>(reinterpret_cast<const unsigned char *>(B.hasU.data()), n) != 0);
...
[[nodiscard]] EIGEN_STRONG_INLINE double complementarity(const dvec &s, const dvec &lmb, double mu, double tau_shift,
                                                         bool use_shifted) {
...
[[nodiscard]] EIGEN_STRONG_INLINE double alpha_ftb_vec(const dvec &x, const dvec &dx, const dvec &s, const dvec &ds,
                                                       const dvec &lmb, const dvec &dlam, const Bounds &B,
                                                       double tau_pri, double tau_dual, double tau_shift = 0.0) {
    using Arrd  = Eigen::Array<double, Eigen::Dynamic, 1>;
    using Arrb  = Eigen::Array<unsigned char, Eigen::Dynamic, 1>;
    using ArrXb = Eigen::Array<bool, Eigen::Dynamic, 1>;

    // ---------- Inequality slacks (primal) ----------
    // Shifted barrier log(s + tau_shift): the slack may fall to -tau_shift, so
    // fraction-to-boundary is on (s + tau_shift), not raw s. Using raw s pins
    // the step at s=0 and stalls infeasible-start inequality problems.
    double a_pri_slacks = 1.0;
    if (s.size() > 0) {
        const Arrd s_a  = s.array() + tau_shift;
        const Arrd ds_a = ds.array();
        // where ds<0, candidate = -(s+sh)/ds (with safe denom), else 1.0
        const Arrd cand = (ds_a < 0.0).select(-(s_a) / ds_a.min(-consts::EPS_DIV), Arrd::Constant(s.size(), 1.0));
    ...
    if (lmb.size() > 0) {
        ...
        const Arrd cand =
            (dlam_a < 0.0).select(-(lmb_a) / dlam_a.min(-consts::EPS_DIV), Arrd::Constant(lmb.size(), 1.0));
    ...
    if (n > 0) {
        ...
        const ArrXb hasL = (Eigen::Map<const Arrb>(reinterpret_cast<const unsigned char *>(B.hasL.data()), n) != 0);
        const ArrXb hasU = (Eigen::Map<const Arrb>(reinterpret_cast<const unsigned char *>(B.hasU.data()), n) != 0);
        ...
        const Arrd infv  = Arrd::Constant(n, std::numeric_limits<double>::infinity());
        ...
        if (!std::isfinite(a_pri_bounds)) a_pri_bounds = 1.0;
    }
...
[[nodiscard]] EIGEN_STRONG_INLINE double comp_inf_norm(const dvec &s, const dvec &lam, const dvec &zL, const dvec &zU,
...
EIGEN_STRONG_INLINE void cap_bound_duals_sigma_box(dvec &zL, dvec &zU, const Bounds &B, bool use_shifted,
...
public:
    [[nodiscard]] EIGEN_STRONG_INLINE Bounds get_bounds(const dvec &x) {
        ...
        Eigen::Map<Arrd>(B.sL.data(), n) = mL.select((xA - lbA).max(consts::EPS_POS), Arrd::Constant(n, 1.0));
        Eigen::Map<Arrd>(B.sU.data(), n) = mU.select((ubA - xA).max(consts::EPS_POS), Arrd::Constant(n, 1.0));
    ...
    InteriorPointStepper(nb::object cfg, ModelC *cfg_model) : cfg_(std::move(cfg)), m_(cfg_model) {
        ...
        std::shared_ptr<Funnel> funnel;
        if (get_attr_or<bool>(cfg_, "use_funnel", false)) {
            FunnelConfig funnel_cfg{
                .funnel_initial_tau   = get_attr_or<double>(cfg_, "funnel_initial_tau", 1.0),
                .funnel_sigma         = get_attr_or<double>(cfg_, "funnel_sigma", 0.1),
                .funnel_delta         = get_attr_or<double>(cfg_, "funnel_delta", 1e-4),
                .funnel_beta          = get_attr_or<double>(cfg_, "funnel_beta", 0.9),
                .funnel_kappa         = get_attr_or<double>(cfg_, "funnel_kappa", 0.9),
                .funnel_min_tau       = get_attr_or<double>(cfg_, "funnel_min_tau", 1e-8),
                .funnel_max_history   = static_cast<std::size_t>(get_attr_or<int>(cfg_, "funnel_max_history", 20)),
                .funnel_kappa_initial = get_attr_or<double>(cfg_, "funnel_kappa_initial", 0.1),
            };
            funnel = std::make_shared<Funnel>(funnel_cfg);
        } else if (get_attr_or<bool>(cfg_, "use_filter", true)) {
            FilterConfig filter_cfg;
            filter_cfg.filter_theta_min   = get_attr_or<double>(cfg_, "filter_theta_min", 1e-4);
            filter_cfg.filter_gamma_theta = get_attr_or<double>(cfg_, "filter_gamma_theta", 0.1);
            filter_cfg.filter_gamma_f     = get_attr_or<double>(cfg_, "filter_gamma_f", 0.1);
            filter_cfg.filter_margin_min  = get_attr_or<double>(cfg_, "filter_margin_min", 1e-8);
            filter_cfg.iter_scale_factor  = get_attr_or<double>(cfg_, "iter_scale_factor", 10.0);
            filter_cfg.filter_max_size    = static_cast<std::size_t>(get_attr_or<int>(cfg_, "filter_max_size", 20));
            filter_cfg.switch_theta       = get_attr_or<double>(cfg_, "switch_theta", 1e-4);
            filter_cfg.switch_f           = get_attr_or<double>(cfg_, "switch_f", 1e-4);
            filter_cfg.tr_delta0          = get_attr_or<double>(cfg_, "tr_delta0", 1.0);
            filter                        = std::make_shared<Filter>(filter_cfg);
        }
        ls_        = std::make_shared<LineSearcher>(cfg_, filter, funnel);
        mg_solver_ = std::make_unique<MehrotraGondzioSolver>(cfg_, m_);
    ...
    std::tuple<dvec, dvec, dvec, SolverInfo> step(const dvec &x, const dvec &lam, const dvec &nu, int it,
        ...
        if (!st.initialized) {
            ...
            const bool have_warm_state = ip_state_opt && ip_state_opt->initialized &&
                                         ip_state_opt->mI == m_->get_mI() && ip_state_opt->mE == m_->get_mE();
            if (have_warm_state) {
                st = *ip_state_opt;
            } else {
        ...
        auto eval_all_at = [&](const dvec &X, int n_, int mI_, int mE_) -> EvalPack {
            m_->eval_all(X);
            EvalPack E;
            ...
            E.theta_I = (mI_ > 0) ? (E.cI.array().max(0.0)).matrix().lpNorm<1>() : 0.0;

            return E;
        };

        auto build_r_d = [&](const EvalPack &E, const dvec &lmb_in, const dvec &nu_in, const dvec &zL_in,
                             const dvec &zU_in) -> dvec {
            dvec r_d = E.g;
            if (E.JI.nonZeros()) r_d.noalias() += E.JI.transpose() * lmb_in;
            if (E.JE.nonZeros()) r_d.noalias() += E.JE.transpose() * nu_in;
            r_d -= zL_in;
            r_d += zU_in;
            return r_d;
        };

        auto aggregated_error = [&](const EvalPack &E, const dvec &r_d, const dvec &zL_in, const dvec &zU_in,
                                    double mu_in, const dvec &s_in, const dvec &lam_in, const dvec &nu_in,
                                    const Bounds &B_in, int n_, int mE_, int mI_, bool use_shifted_, double tau_shift_,
            ...
            const double sum_mults = lam_in.lpNorm<1>() + nu_in.lpNorm<1>() + zL_in.lpNorm<1>() + zU_in.lpNorm<1>();
            const double s_d       = clamp_min(sum_mults / clamp_min(denom_ct, 1), s_max) / s_max;
            const double s_c = clamp_min((zL_in.lpNorm<1>() + zU_in.lpNorm<1>()) / clamp_min(n_, 1), s_max) / s_max;
            const double comp_box =
                detail::comp_inf_norm(s_in, lam_in, zL_in, zU_in, B_in, mu_in, use_shifted_, tau_shift_, bound_shift_);
            const double ineq_violation = (mI_ > 0) ? safe_inf_norm((E.cI.array().max(0.0)).matrix()) : 0.0;
            return std::max(
                {safe_inf_norm(r_d) / s_d, (mE_ > 0) ? safe_inf_norm(E.cE) : 0.0, ineq_violation, comp_box / s_c});
        };

        auto assemble_W = [&](const spmat &H_in, const dvec &Sigma_x, const std::optional<spmat> &JI_opt,
                              const dvec &Sigma_s_opt) -> spmat {
            spmat W = H_in;
            if (Sigma_x.size()) W.diagonal().array() += Sigma_x.array();
        ...
        auto step_norm_W = [&](const dvec &dx, const dvec &ds, const Sigmas &Sg, dvec &Wdx_scratch) -> double {
            Wdx_scratch.setZero(dx.size());
            m_->getCompiledWOp().perform_op(dx, Wdx_scratch);
            const double qx = dx.dot(Wdx_scratch);
            const double qs = (Sg.Sigma_s.size() && ds.size()) ? ds.cwiseProduct(Sg.Sigma_s).dot(ds) : 0.0;
            return std::sqrt(std::max(0.0, qx + qs));
        };

        auto scale_full_step = [&](double sc, dvec &dx, dvec &dnu, dvec &ds, dvec &dlam, dvec &dzL, dvec &dzU) {
            if (sc >= 1.0) return;
            dx *= sc;
            if (dnu.size()) dnu *= sc;
            if (ds.size()) ds *= sc;
            if (dlam.size()) dlam *= sc;
            dzL *= sc;
            dzU *= sc;
        };
        ...
        auto barrier_sum = [&](const dvec &sv) {
            ...
            double           v   = 0.0;
            for (int i = 0; i < sv.size(); ++i) v += std::log(std::max(sv[i], eps));
            return v;
        };

        auto bound_barrier_sum = [&](const Bounds &Bv) {
            constexpr double eps = 1e-12;
            double           v   = 0.0;
            for (int i = 0; i < Bv.sL.size(); ++i) {
                if (Bv.hasL[i]) v += std::log(std::max(Bv.sL[i], eps));
                if (Bv.hasU[i]) v += std::log(std::max(Bv.sU[i], eps));
            }
            return v;
        };
        ...
        auto update_penalty_parameters = [&](double theta_current, double theta_reduction_rate, double mu_current,
            ...
            if (theta_history_.size() > 5) theta_history_.pop_front();
            ...
            if (theta_history_.size() >= 3) {
                for (size_t i = 1; i < theta_history_.size(); ++i) {
                    if (theta_history_[i - 1] > 1e-16) {
                        avg_reduction += (theta_history_[i - 1] - theta_history_[i]) / theta_history_[i - 1];
                    }
                }
            ...
            penalty_rho_                  = std::max(penalty_rho_, min_rho_relative * mu_current);
        ...
        auto predicted_reduction_cubic = [&](const dvec &dx, const dvec &r_d, dvec &Wdx_scratch, const dvec &s,
            ...
            m_->getCompiledWOp().perform_op(dx, Wdx_scratch);
            const double q   = 0.5 * alpha_step * alpha_step * dx.dot(Wdx_scratch);
            const double lin = alpha_step * (r_d.size() ? r_d.dot(dx) : 0.0);
            ...
            if (s.size() && ds.size()) {
                ...
                for (int i = 0; i < s.size(); ++i) {
                    ...
                    const double sip = std::max(s[i] + alpha_step * ds[i], eps);
            ...
            for (int i = 0; i < dx.size(); ++i) {
                constexpr double eps = 1e-12;
                if (B_cur.hasL[i]) {
                    const double sL      = std::max(B_cur.sL[i], eps);
                    const double sL_next = std::max(B_cur.sL[i] + alpha_step * dx[i], eps);
                    dphi += std::log(sL) - std::log(sL_next);
                }
                if (B_cur.hasU[i]) {
                    const double sU      = std::max(B_cur.sU[i], eps);
                    const double sU_next = std::max(B_cur.sU[i] - alpha_step * dx[i], eps);
                    dphi += std::log(sU) - std::log(sU_next);
                }
            }
            ...
            if (mE > 0 && E.cE.size() && E.JE.nonZeros()) {
                dvec JE_dx = E.JE * (alpha_step * dx);
                theta_pred_linear += E.cE.array().sign().matrix().dot(JE_dx);
            }

            if (mI > 0 && E.cI.size() && E.JI.nonZeros()) {
                dvec JI_dx = E.JI * (alpha_step * dx);
                theta_pred_linear += (E.cI.array() + JI_dx.array()).max(0.0).sum() - E.cI.array().max(0.0).sum();
            }
        ...
        if (err_prev <= tol && penalty_converged) {
            ...
            info.f             = E.f;
            info.theta         = E.theta;
            info.stat          = safe_inf_norm(r_d);
            info.ineq          = (mI > 0) ? safe_inf_norm((E.cI.array().max(0.0)).matrix()) : 0.0;
        ...
        auto [alpha_aff, mu_aff, sigma_pred, step] = mg_solver_->solve(
            W, r_d, JE_eff, r_pE, (mI > 0 && E.JI.nonZeros()) ? std::optional<spmat>(E.JI) : std::nullopt, r_pI, s, lmb,
            zL, zU, B, use_shifted, tau_shift, bound_shift, mu, E.theta, Sg);

        dvec dx   = step.dx;
        dvec dnu  = (mE > 0 && step.dnu.size() == mE) ? step.dnu : dvec::Zero(mE);
        dvec ds   = step.ds;
        dvec dlam = step.dlam;
        dvec dzL  = step.dzL;
        dvec dzU  = step.dzU;
        ...
        const double stepW = step_norm_W(dx, (mI ? ds : dvec()), Sg, Wdx_scratch);
        ...
        const double min_step_norm      = get_attr_or<double>(cfg_, "ip_min_step_norm", 1e-15);
        ...
        if (stepW < min_step_norm) {
            ...
            tr_clip_streak_ = std::max(0, tr_clip_streak_ - 1);
        ...
        double d_phi = E.g.dot(dx);
        if (mI > 0 && ds.size() == s.size()) {
            // Shifted-barrier directional derivative: d/ds[-mu*log(s+sh)] = -mu/(s+sh).
        ...
        kkt_new.ineq = (mI > 0) ? safe_inf_norm((En.cI.array().max(0.0)).matrix()) : 0.0;
        kkt_new.eq   = (mE > 0) ? safe_inf_norm(En.cE) : 0.0;
        kkt_new.comp =
            detail::comp_inf_norm(s_new, lmb_new, zL_new, zU_new, Bn, mu, use_shifted, tau_shift, bound_shift);
        ...
        const double pred_threshold = get_attr_or<double>(cfg_, "ip_pred_threshold", 1e-16);
        double       rho;
        if (std::abs(pred) < pred_threshold) {
            // Use constraint progress as fallback indicator
            const double theta_progress = (E.theta > 1e-16) ? std::max(0.0, (E.theta - En.theta) / E.theta) : 1.0;
            rho                         = (ared > 0 && alpha > 0.1) ? std::min(0.9, 0.5 + theta_progress) : 0.1;
        } else {
            rho = ared / pred;
        }

        // Adaptive trust region parameters based on penalty state
        const double eta_excellent = get_attr_or<double>(cfg_, "ip_tr_eta_excellent", 0.9);
        const double eta_good      = get_attr_or<double>(cfg_, "ip_tr_eta_good", 0.7);
        const double eta_poor      = get_attr_or<double>(cfg_, "ip_tr_eta_poor", 0.15);

        // Adaptive expansion/contraction based on constraint violation level
        double gam_expand = get_attr_or<double>(cfg_, "ip_tr_gamma_expand", 2.0);
        double gam_shrink = get_attr_or<double>(cfg_, "ip_tr_gamma_shrink", 0.6);
        ...
        if (E.theta < 1e-6) {
            ...
            gam_shrink *= 0.8; // More aggressive shrinkage when highly infeasible
        }
        ...
        if (rho > eta_excellent && near_full_alpha && unclipped && tr_good_streak_ >= 2) {
            // Excellent step with consistent good performance
            tr_new *= gam_expand;
            tr_good_streak_++;
        } else if (rho > eta_good && near_full_alpha && good_constraint_progress) {
            // Good step with constraint progress
            tr_new *= std::sqrt(gam_expand);
            tr_good_streak_++;
        } else if (rho < eta_poor || alpha <= ls_min_alpha || tr_clip_streak_ >= 3) {
            // Poor step or persistent clipping
            tr_new *= gam_shrink;
            tr_good_streak_ = 0;
        } else if (tr_clip_streak_ >= 2 && !good_constraint_progress) {
        ...
        const double mu_next_meas = mg_solver_->average_complementarity(s_new, lmb_new, zL_new, zU_new, Bn, use_shifted,
                                                                        tau_shift, bound_shift);
        st.mu                     = std::max(mu_next_meas, get_attr_or<double>(cfg_, "ip_mu_min", 1e-12));
        ...
        info.good_streak        = tr_good_streak_;
        info.theta_reduction    = (E.theta > 1e-16) ? (E.theta - En.theta) / E.theta : 0.0;
    ...
    int    tr_clip_streak_ = 0;
    int    tr_good_streak_ = 0;
...
private:
    ...
    std::optional<spmat>                  cached_eq_jac_;
    std::string                           cached_kkt_method_;
    bool                                  kkt_factorization_valid_ = false;

    void reset_kkt_cache_() {
        cached_kkt_solver_.reset();
        cached_kkt_matrix_.resize(0, 0);
        cached_eq_jac_.reset();
        cached_kkt_method_.clear();
        kkt_factorization_valid_ = false;
    }

    bool matrices_equal(const spmat &A, const spmat &B, double tol = 1e-9) {
        if (A.rows() != B.rows() || A.cols() != B.cols() || A.nonZeros() != B.nonZeros() ||
            A.outerSize() != B.outerSize()) {
            return false;
        }
        const int *a_outer = A.outerIndexPtr();
        const int *b_outer = B.outerIndexPtr();
        const int *a_inner = A.innerIndexPtr();
        const int *b_inner = B.innerIndexPtr();
        for (int i = 0; i < A.outerSize() + 1; ++i) {
            if (a_outer[i] != b_outer[i]) return false;
        }
        for (int i = 0; i < A.nonZeros(); ++i) {
            if (a_inner[i] != b_inner[i]) return false;
            if (std::abs(A.valuePtr()[i] - B.valuePtr()[i]) > tol) return false;
        }
        return true;
    }

    bool optional_matrices_equal(const std::optional<spmat> &A, const std::optional<spmat> &B, double tol = 1e-9) {
        if (A.has_value() != B.has_value()) return false;
        if (!A.has_value()) return true;
        return matrices_equal(*A, *B, tol);
    }

    void load_defaults_() {
        auto set_if_missing = [&](const char *name, nb::object v) {
            if (!pyu::has_attr(cfg_, name)) cfg_.attr(name) = v;
        };
        ...
        set_if_missing("ip_slack_init_min", nb::float_(1e-6));
        set_if_missing("ip_slack_init_pad", nb::float_(1e-3));
        ...
        set_if_missing("ls_backtrack", nb::float_(get_attr_or<double>(cfg_, "ip_alpha_backtrack", 0.5)));
        set_if_missing("ls_armijo_f", nb::float_(get_attr_or<double>(cfg_, "ip_armijo_coeff", 1e-4)));
        set_if_missing("ls_max_iter", nb::int_(get_attr_or<int>(cfg_, "ip_ls_max", 5)));
        set_if_missing("ls_min_alpha", nb::float_(get_attr_or<double>(cfg_, "ip_alpha_min", 1e-10)));
    }
    ...
    [[nodiscard]] IPState state_from_model_(const dvec &x) {
        ...
        if (s.mI > 0) {
            ...
            for (int i = 0; i < s.mI; ++i) {
                const bool   violated   = cI[i] > 0.0;
                const double slack_seed = violated ? cI[i] : -cI[i];
                s.s[i]                  = clamp_min(slack_seed + slack_init_pad, slack_init_min);
                const double dual_seed  = violated ? slack_init_pad : s.s[i];
                const double denom      = (tau_shift > 0.0) ? (dual_seed + tau_shift) : dual_seed;
        ...
        for (int i = 0; i < x.size(); ++i) {
            if (B.hasL[i]) s.zL[i] = clamp_min(mu0 / clamp_min(B.sL[i] + bound_shift, 1e-12), 1e-8);
            if (B.hasU[i]) s.zU[i] = clamp_min(mu0 / clamp_min(B.sU[i] + bound_shift, 1e-12), 1e-8);
        }
    ...
    [[nodiscard]] KKTResult solve_KKT_(const spmat &W, const dvec &rhs_x, const std::optional<spmat> &JE,
                                       const std::optional<dvec> &rpE, std::string_view method_in) {
        ...
        const bool can_reuse =
            ...
            optional_matrices_equal((mE > 0 ? std::optional<spmat>(*JE) : std::nullopt), cached_eq_jac_);
        if (can_reuse) {
        ...
        auto [dx, dy, reusable] =
            strat->factor_and_solve(m_, W, (mE > 0 ? std::optional<spmat>(*JE) : std::nullopt), r1, r2, conf,
                                    std::nullopt, kkt_cache_, 0.0, std::nullopt, true, true);

        cached_kkt_matrix_       = W;
        cached_eq_jac_           = (mE > 0 ? std::optional<spmat>(*JE) : std::nullopt);
        cached_kkt_method_       = method_cpp;
