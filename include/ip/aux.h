// Enhanced interior-point auxiliaries with SOTA improvements
...
class RichardsonExtrapolator {
   private:
    ...
    int max_hist_ = 3;
    double min_ratio_ = 1.5,
           max_ratio_ = 4.0;     // require roughly geometric spacing
    ...
    static std::optional<double> estimate_p_pairwise(
        const std::vector<double>& h, const std::vector<dvec>& f) {
        ...
        std::nth_element(pvals.begin(), pvals.begin() + pvals.size() / 2,
                         pvals.end());
        return pvals[pvals.size() / 2];
    }

    static bool ratios_reasonable(const std::vector<double>& h, double rmin,
                                  double rmax) {
        ...
        for (int i = 1; i < m; ++i) {
            for (int j = 1; j <= i; ++j) {
                ...
                if (ampl > max_amplify_) {
                    ...
                    R[i][j] =
                        R[i][j - 1] + (R[i][j - 1] - R[i - 1][j - 1]) / denom;
   ...
   public:
    ...
    void add_step(const dvec& dx, double h,
    ...
    ExtrapolatedStep extrapolate_step(const dvec& current_dx, double current_h,
                                      double tol = 1e-8) {
        ...
        std::vector<double> h;
        std::vector<dvec> f;
        h.reserve(hist_.size() + 1);
        f.reserve(hist_.size() + 1);
        for (auto& s : hist_)
            if (s.dx.size() == current_dx.size()) {
                h.push_back(s.h);
                f.push_back(s.dx);
            }
        ...
        std::stable_sort(idx.begin(), idx.end(),
                         [&](size_t a, size_t b) { return h[a] < h[b]; });
        ...
        if (!strictly_increasing(hs) ||
            !ratios_reasonable(hs, min_ratio_, max_ratio_)) {
