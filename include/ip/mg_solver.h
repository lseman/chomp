// mg_solver.h — Mehrotra predictor-corrector with Gondzio corrections
#pragma once
...
class MehrotraGondzioSolver {
    ...
    explicit MehrotraGondzioSolver(nb::object cfg, ModelC* model)
        : cfg_(cfg), model_(model),
          soc_(SOC::Config{get_attr_or<double>(cfg_, "soc_threshold", 0.3),
                           get_attr_or<double>(cfg_, "soc_tau", 0.995)}) {
        load_config_();
    }

    std::tuple<double, double, double, StepData> solve(
        ...
        soc_.apply(final_step, W, JE, JI, s, lam, zL, zU, B, alpha_aff, mu_target,
                   use_shifted, tau_shift, bound_shift, Sg,
   ...
   private:
    ...
    double prev_centrality_ = std::numeric_limits<double>::infinity();
    ...
    double compute_barrier_parameter_(double mu, double mu_aff, double sigma,
        ...
        switch (config_.barrier_strategy) {
            ...
            case BarrierStrategy::MehrotraSafeguarded: {
                ...
                // Prevent too aggressive reduction
                if (mu_target < config_.safeguard_threshold * mu &&
                    alpha_aff < 0.9) {
                    mu_target =
                        std::max(mu_target, config_.safeguard_threshold * mu);
                }
                // Duality gap monotonicity safeguard: if mu_aff >= mu,
                // the affine step would increase the duality gap. Fall back
                // to a conservative target to avoid divergence.
                if (mu_aff >= mu) {
                    mu_target = std::max(mu_target, 0.5 * mu);
                }
                return mu_target;
            }
    ...
    std::pair<double, double> compute_step_lengths_(
    ...
    StepData apply_gondzio_corrections_(
        ...
        if (config_.adaptive_correction_count) {
            ...
            if (centrality > prev_centrality_ * 0.95) {
                correction_stall_count_++;
                if (correction_stall_count_ > 2)
                    max_corr = std::max(1, max_corr - 1);
            } else {
                correction_stall_count_ = 0;
                if (centrality < 0.1)
                    max_corr =
                        std::min(config_.max_corrections + 1, max_corr + 1);
            }
            prev_centrality_ = centrality;
        }
        ...
        for (int k = 0; k < max_corr; ++k) {
            ...
            if (!should_apply_gondzio_(centrality_current, alpha_max, k)) break;
            ...
            if (corr_norm > config_.safeguard_threshold * base_norm &&
                ...
                corr_res.dx *= scale;
                if (corr_res.dy.size() > 0) corr_res.dy *= scale;
            }

            // Apply correction
            result.dx += corr_res.dx;
            if (JE && corr_res.dy.size() > 0) result.dnu += corr_res.dy;
            ...
            if (new_alpha < alpha_max * 0.95) break;
        }
    ...
    std::pair<dvec, dvec> compute_correction_rhs_(
        ...
        dvec rhs_x(n);
        dvec rhs_s(mI);
        rhs_x.setZero();  // Only if necessary; consider removing if all
   ...
   public:
    double average_complementarity(
        const dvec& s,    // size mI
