...
// Key improvements: SIMD, better caching, randomized eigenvalue estimation,
// memory management
   ...
   public:
    ...
    static std::tuple<double, double, bool> block_lanczos_bounds(
        ...
        for (int iter = 0; iter < max_iter; ++iter) {
            ...
            if (iter > 0) {
                T.block(start - block_size, start, block_size, block_size) =
                    T.block(start, start - block_size, block_size, block_size)
                        .transpose();
            }
            ...
            if (iter > 0) {
                ...
                beta_full -= Q * T.block(start, start - block_size, block_size,
                                         block_size);
            }
        ...
        Eigen::SelfAdjointEigenSolver<dmat> es(
            T.topLeftCorner(std::min((int)T.rows(), block_size * max_iter),
...
class Regularizer {
   ...
   public:
    ...
    std::pair<spmat, RegInfo> regularize(
        const spmat& H_in, int iteration = 0,
        ...
        if (min_eig < -cfg_.min_eig_thresh) {
            ...
            info.mode = "TIKHONOV";
            double reg_param =
                compute_regularization_parameter_(min_eig, max_eig, cond);
    ...
    std::pair<dmat, RegInfo> regularize_auto(const dmat& H_in) const {
        ...
        if (min_eig < -1e-10) {
            ...
            info.mode = "TIKHONOV";
            H_reg =
                H + compute_regularization_parameter_(min_eig, max_eig, cond) *
   ...
   private:
    ...
    bool is_symmetric_small_(const spmat& A) const {
        for (int k = 0; k < A.outerSize(); ++k) {
            for (spmat::InnerIterator it(A, k); it; ++it) {
                if (std::abs(it.value() - A.coeff(it.col(), it.row())) >
                    1e-12) {
                    return false;
                }
            }
    ...
    std::tuple<double, double, bool> get_spectral_bounds_(
        const spmat& H) const {
        if (cfg_.enable_cache) {
    ...
    std::tuple<double, double, bool> compute_spectral_bounds_(
        const spmat& H) const {
        const int n = H.rows();
        if (n == 0) return {0.0, 0.0, true};
        ...
        } else if (n < cfg_.large_matrix_threshold &&
                   cfg_.use_randomized_bounds) {
    ...
    std::tuple<double, double, bool> compute_exact_bounds_(
        const spmat& H) const {
#if AD_HAVE_SPECTRA
        try {
            ...
            int nev = 1;
            int ncv =
                std::min(n, std::max(4, cfg_.spectra_ncv_factor * nev + 1));
            Spectra::SymEigsSolver<Spectra::SparseSymMatProd<double>> eigs_max(
                op, nev, ncv);
            eigs_max.init();
            eigs_max.compute(Spectra::SortRule::LargestAlge, cfg_.spectra_maxit,
                             cfg_.spectra_tol);
            ...
            Spectra::SymEigsShiftSolver<Spectra::SparseSymShiftSolve<double>>
                eigs_min(op_shift, nev, ncv, 0.0);
            eigs_min.init();
            eigs_min.compute(Spectra::SortRule::SmallestAlge,
                             cfg_.spectra_maxit, cfg_.spectra_tol);
    ...
    std::tuple<double, double, bool> compute_approximate_bounds_(
        const spmat& H) const {
        ...
        // reduction(max:max_diag) if(cfg_.enable_parallel)
        for (int i = 0; i < n; ++i) {
        ...
        // #pragma omp parallel for reduction(min:min_gershgorin)
        // reduction(max:max_gershgorin) if(cfg_.enable_parallel)
        for (int i = 0; i < n; ++i) {
    ...
    void adapt_sigma_(double min_eig, double max_eig, double cond,
        ...
        } else if (cond < cfg_.target_cond * 0.1 &&
                   min_eig > cfg_.min_eig_thresh) {
    ...
    double compute_regularization_parameter_(double min_eig, double max_eig,
                                             double cond) const {
        if (max_eig <= 0.0) return sigma_;

        double target_min = max_eig / cfg_.target_cond;
        double reg_param =
            std::max({sigma_, cfg_.min_eig_thresh, target_min - min_eig});
    ...
    // Eigenvalue modification
    spmat eigen_modification_(const spmat& H, double min_eig,
                              double max_eig) const {
        const int n = H.rows();
    ...
    spmat eigen_modification_low_rank_(const spmat& H,
                                       double target_min) const {
        const int n = H.rows();
        ...
        try {
            Spectra::SparseSymShiftSolve<double> op_shift(H);
            int ncv = std::min(
                n, std::max(2 * nev + 1, cfg_.spectra_ncv_factor * nev + 1));
            Spectra::SymEigsShiftSolver<Spectra::SparseSymShiftSolve<double>>
                eigs(op_shift, nev, ncv, 0.0);

            eigs.init();
            eigs.compute(Spectra::SortRule::SmallestAlge,
                         cfg_.spectra_maxit / 2, cfg_.spectra_tol * 2);
    ...
    std::pair<double, double> power_iteration_bounds(const dmat& H,
        ...
        if (lu.isInvertible()) {
            ...
            for (int i = 0; i < max_iter; ++i) {
                u = lu.solve(u);
                min_inv_eig = u.norm();
                if (min_inv_eig > 1e-16)
                    u /= min_inv_eig;
                else
                    break;
            }
            double min_eig =
                (min_inv_eig > 1e-16) ? (1.0 / min_inv_eig + shift) : shift;
            return {min_eig, max_eig};
        } else {
...
#pragma omp simd
        ...
        return es.eigenvectors() * ev.asDiagonal() *
...
#pragma omp simd
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (spmat::InnerIterator it(A, i); it; ++it) {
                ...
                const double strength =
                    std::abs(it.value()) / (sqrt_diag_i * sqrt_diag(j) + 1e-16);
                if (strength >= thresh) {
                    agg(j) = cur_agg;
                    marked[j] = 1;
                }
    ...
    static spmat build_prolongation_(const Eigen::VectorXi& agg, int n,
                                     int nc) {
    ...
    dvec jacobi_sweep_(const spmat& A, const dvec& D_inv, dvec x, const dvec& b,
                       double omega, int iters) const {
        const int n = A.rows();
...
class Lanczos {
   ...
   public:
    static LanczosResult estimate_extreme_eigenvalue(const spmat& A,
                                                     bool largest = true,
                                                     int max_iter = 30,
                                                     double tol = 1e-4) {
        const int n = A.rows();
        ...
        for (int k = 0; k < max_iter; ++k) {
            ...
            if (k > 5 && std::abs(theta - theta_prev) <
                             tol * (std::abs(theta) + 1e-16)) {
