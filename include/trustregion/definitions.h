...
[[nodiscard]] inline double boundary_intersection_euclid(
    const dvec& p, const dvec& d, double Delta) noexcept {
    const double pTp = p.squaredNorm();
    const double pTd = p.dot(d);
    const double dTd = d.squaredNorm();
    if (dTd <= kTiny) return 0.0;
...
[[nodiscard]] inline double boundary_intersection_metric(const Metric& M,
                                                         ...
                                                         const dvec& d,
                                                         double Delta) {
    if (!M.valid) return boundary_intersection_euclid(p, d, Delta);
...
template <class Mat>
dvec safe_diag(const Mat& H) {
    dvec d = H.diagonal();
    d = d.unaryExpr([](double v) { return (std::abs(v) > 0) ? v : 1.0; });
    return d;
}
template <>
inline dvec safe_diag<spmat>(const spmat& H) {
    dvec d(H.rows());
    d.setOnes();
...
struct PrecJacobi {
    template <class Mat>
    [[nodiscard]] static Prec from(const Mat& H) {
        Prec P;
    ...
    [[nodiscard]] static Prec fromSparseSPD(const spmat& H,
        ...
        P.apply = [=](const dvec& r, dvec& z) {
            ...
            for (int k = n - 1; k >= 0; --k) {
                for (spmat::InnerIterator it(H, k); it; ++it) {
                    const int i = it.row(), j = it.col();
                    if (i < j) z[i] -= omega * it.value() * z[j];
                }
                z[k] /= (std::abs(D[k]) > 1e-30 ? D[k] : 1.0);
            }
            const double scale = 1.0 / (omega * (2.0 - omega));
            if (std::isfinite(scale) && std::abs(scale) > 1e-16) z *= scale;
        };
        return P;
    }
};
...
[[nodiscard]] Prec make_prec(const Mat* H, const std::string& kind,
    ...
    if (!use_prec) return PrecIdentity::make(n);
    if (kind == "identity") return PrecIdentity::make(n);
    if constexpr (std::is_same_v<Mat, spmat>) {
        if (H && kind == "ssor") return PrecSSOR::fromSparseSPD(*H, ssor_omega);
    }
    if (H) return PrecJacobi::from(*H);
