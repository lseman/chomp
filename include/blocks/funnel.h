...
class Funnel {
   public:
    explicit Funnel(FunnelConfig cfg)
        : cfg_(std::move(cfg)), tau_(cfg_.funnel_initial_tau), iter_(0) {
        cfg_.validate();
        history_.reserve(cfg_.funnel_max_history +
                         1);  // Pre-allocate to avoid reallocations
    }

    // Default constructor with sensible defaults
    explicit Funnel()
        : Funnel(FunnelConfig{.funnel_initial_tau = 1.0,
                              .funnel_sigma = 0.1,
                              .funnel_delta = 1e-4,
                              .funnel_beta = 0.9,
                              .funnel_kappa = 0.9,
    ...
    [[nodiscard]] static constexpr std::optional<double> quad_pred_df(
        const std::optional<double>& gTs,
        const std::optional<double>& sTHs) noexcept {
        if (!gTs || !sTHs) return std::nullopt;
        return std::max(0.0, -(*gTs + 0.5 * (*sTHs)));
    }

    // Optimized ratio helper with better numerical stability
    [[nodiscard]] static constexpr double rho(double actual, double predicted,
    ...
    [[nodiscard]] bool is_acceptable(
        double current_theta, double current_f, double new_theta, double new_f,
        ...
        if (new_theta < 0.0 || current_theta < 0.0 || !std::isfinite(new_f) ||
            new_theta > tau_ + eps) {
            return false;
        }
        ...
        const bool f_type =
            (pred_df_use >=
             cfg_.funnel_delta * (current_theta * current_theta) - eps);
        ...
        if (f_type) {
            ...
            return armijo_ok &&
                   (rho(df_act, pred_df_use) >= cfg_.funnel_sigma_rho_f);
        } else {
            // h-type step: constraint reduction with optional curvature
            // enhancement
            double pred_dtheta_use = predicted_dtheta_lin;
            if (JTJs_s2 && *JTJs_s2 > 0.0) {
                pred_dtheta_use +=
                    cfg_.funnel_theta_curv_scale * 0.5 * std::sqrt(*JTJs_s2);
            }
    ...
    bool add_if_acceptable(double current_theta, double current_f,
                           double new_theta, double new_f,
                           double predicted_df_lin, double predicted_dtheta_lin,
                           std::optional<double> gTs = std::nullopt,
                           std::optional<double> sTHs = std::nullopt,
        ...
        if (iter_ == 0) {
            tau_ = std::max(
                tau_, cfg_.funnel_kappa_initial * std::max(current_theta, 0.0));
        }

        // Check acceptance
        if (!is_acceptable(current_theta, current_f, new_theta, new_f,
                           predicted_df_lin, predicted_dtheta_lin, gTs, sTHs,
                           JTJs_s2)) {
            return false;
        }
        ...
        if (cfg_.funnel_max_history > 0) {
            if (history_.size() >= cfg_.funnel_max_history) {
                // Use move-based rotation instead of erase for better
                // performance
                std::rotate(history_.begin(), history_.begin() + 1,
                            history_.end());
        ...
        const bool f_type =
            (pred_df_use >=
             cfg_.funnel_delta * (current_theta * current_theta) - 1e-10);

        // Update tau only for h-type steps
        if (!f_type) {
            const double curv = std::max(0.0, JTJs_s2.value_or(0.0));
            const double phi =
                1.0 / (1.0 + cfg_.funnel_phi_alpha * std::sqrt(curv));
            const double kappa_eff =
                std::clamp(cfg_.funnel_kappa * phi, 0.0, 1.0);
    ...
    [[nodiscard]] const std::vector<std::pair<double, double>>& history()
        const noexcept {
        return history_;
    }
