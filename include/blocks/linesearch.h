// line_searcher_improved.cc — faster, allocation-light line search with
// mask/view fast paths Compile-time toggles (define if your ModelC supports
// them):
//   - MODEL_HAS_EVAL_MASK : enables eval_all_mask(x, mask) instead of string
//   vectors
//   - MODEL_HAS_VIEW      : enables get_view() to avoid optionals/copies
//
// Notes:
// - SOC uses sparse least-squares (SparseQR) on [JE; JI] without dense
// conversion.
// - Fraction-to-boundary and slack checks are vectorized.
// - Non-monotone memory uses a fixed-size ring buffer (no per-iteration
// allocs).
...
static inline double compute_theta(const dvec* cE, const dvec* cI,
                                   const dvec& s, bool inf_norm = false) {
    double tE = 0.0, tI = 0.0;
    if (cE && cE->size() > 0)
        tE = inf_norm ? cE->lpNorm<Eigen::Infinity>() : cE->lpNorm<1>();
...
class RingBuffer {
   public:
    explicit RingBuffer(int cap = 5)
        : cap_(std::max(1, cap)), buf_(cap_, 0.0) {}
    void reset(int cap) {
        cap_ = std::max(1, cap);
...
class LineSearcher {
   public:
    LineSearcher(nb::object cfg, std::shared_ptr<Filter> filter = nullptr,
        ...
        cfg_.ip_fraction_to_boundary_tau = std::clamp(
            getattr_or_double(cfg_obj_, "ip_fraction_to_boundary_tau", 0.995),
            0.9, 0.999);
        cfg_.ls_theta_restoration = std::max(
            1.0, getattr_or_double(cfg_obj_, "ls_theta_restoration", 1e3));
        ...
        const std::string mode =
            nb::hasattr(cfg_obj_, "ls_merit_mode")
                ? nb::cast<std::string>(cfg_obj_.attr("ls_merit_mode"))
                : std::string("filter");
        if (mode == "monotone")
            cfg_.merit_mode = LSMeritMode::MonotonePhi;
        else if (mode == "nonmonotone")
            cfg_.merit_mode = LSMeritMode::NonMonotonePhi;
        else
            cfg_.merit_mode = LSMeritMode::FilterOrFunnel;

        cfg_.nm_memory =
            std::max(1, getattr_or_int(cfg_obj_, "ls_nm_memory", 5));
        cfg_.nm_safeguard =
            std::max(0.0, getattr_or_double(cfg_obj_, "ls_nm_safeguard", 0.0));

        cfg_.use_richardson =
            getattr_or_bool(cfg_obj_, "ls_use_richardson", true);
        cfg_.rich_tol =
            std::max(1e-14, getattr_or_double(cfg_obj_, "ls_rich_tol", 1e-8));
        cfg_.rich_min_order =
            std::max(2, getattr_or_int(cfg_obj_, "ls_rich_min_order", 2));

        cfg_.ls_sufficient_decrease = std::max(
            1e-8, getattr_or_double(cfg_obj_, "ls_sufficient_decrease", 1e-4));
        cfg_.ls_curvature_condition = std::clamp(
            getattr_or_double(cfg_obj_, "ls_curvature_condition", 0.9),
            cfg_.ls_sufficient_decrease, 0.999);
        cfg_.use_strong_wolfe =
            getattr_or_bool(cfg_obj_, "ls_use_strong_wolfe", false);
        cfg_.ls_theta_eps =
            std::max(1e-16, getattr_or_double(cfg_obj_, "ls_theta_eps", 1e-12));
        cfg_.ls_watchdog_max =
            std::max(1, getattr_or_int(cfg_obj_, "ls_watchdog_max", 10));
        cfg_.ls_theta_min_improvement = std::clamp(
            getattr_or_double(cfg_obj_, "ls_theta_min_improvement", 0.1), 0.01,
            0.9);
    ...
    std::tuple<double, int, bool, dvec, dvec> search(
        ...
        if (ds.size() == s.size() && s.size() > 0) {
            ...
            if (mask.any()) {
                ...
                for (Eigen::Index i = 0; i < ratio.size(); ++i)
                    if (mask[i] != 0.0)
                        min_ratio = std::min(min_ratio, ratio[i]);
                if (std::isfinite(min_ratio))
                    alpha_ftb = std::min(alpha_ftb, min_ratio);
            }
        }
        ...
        if (cE0 && cE0->size() > 0) {
            dvec je_dx =
                (JE0 && JE0->rows() > 0) ? (*JE0) * dx : Z_(cE0->size());
            dvec cE_pred = *cE0 + je_dx;
            theta_pred += cE_pred.lpNorm<1>();
        }
        if (cI0 && cI0->size() > 0) {
            dvec ji_dx =
                (JI0 && JI0->rows() > 0) ? (*JI0) * dx : Z_(cI0->size());
        ...
        while (it < cfg_.ls_max_iter && watchdog < cfg_.ls_watchdog_max) {
            ...
            if (funnel_) {
                if (funnel_->is_acceptable(theta0, f0, theta_t, f_t, pred_df,
                                           pred_dtheta)) {
                    funnel_->add_if_acceptable(theta0, f0, theta_t, f_t,
                                               pred_df, pred_dtheta);
                    push_phi_(phi_t);
            ...
            if (cfg_.use_richardson && it < cfg_.ls_max_iter / 2) {
                ...
                if (scratch_dxds_.size() != dx.size() + ds.size())
                    scratch_dxds_.resize(dx.size() + ds.size());
                scratch_dxds_.head(dx.size()) = alpha * dx;
                scratch_dxds_.tail(ds.size()) = alpha * ds;
                rich.add_step(scratch_dxds_, alpha);
                auto ext =
                    rich.extrapolate_step(scratch_dxds_, alpha, cfg_.rich_tol);
                if (ext.order_achieved >= cfg_.rich_min_order &&
                    ...
                    if (ds_ref.size() == s.size() && s.size() > 0) {
                        Eigen::ArrayXd mask =
                            (ds_ref.array() < -cfg_.ls_theta_eps)
                                .cast<double>();
                        if (mask.any()) {
                            Eigen::ArrayXd ratio =
                            ...
                            for (Eigen::Index i = 0; i < ratio.size(); ++i)
                                if (mask[i] != 0.0)
                                    min_ratio = std::min(min_ratio, ratio[i]);
                            if (std::isfinite(min_ratio))
                                alpha_ref = std::min(alpha_ref, min_ratio);
                        }
                        ...
                        if (eval_step_(model, x_ref)) {
                            ...
                            if (std::isfinite(f_ref)) {
                                ...
                                if (s_ref.size() > 0) {
                                    phi_ref_acc -=
                                        mu *
                                        (s_ref.array().unaryExpr([&](double v) {
                                            return safe_log_barrier(
                                                v, barrier_eps);
                                        })).sum();
                                }
                                if (std::isfinite(phi_ref_acc)) {
                                    ...
                                    const double theta_ref = compute_theta(
                                        cE_r, cI_r, s_ref.size() ? s_ref : s);
                                    bool ok = false;
                                    if (funnel_) {
                                        ok = funnel_->is_acceptable(
                                            theta0, f0, theta_ref, f_ref,
                                            pred_df, pred_dtheta);
                                        if (ok)
                                            funnel_->add_if_acceptable(
                                                theta0, f0, theta_ref, f_ref,
                                                pred_df, pred_dtheta);
                                    } else if (filter_) {
                                        ...
                                        if (cfg_.merit_mode ==
                                                LSMeritMode::NonMonotonePhi &&
                                            !phi_hist_.empty())
                                            phi_ref_cmp = phi_hist_.max() +
                                                          cfg_.nm_safeguard;
                                        ok = (phi_ref_acc <=
                                              phi_ref_cmp +
                                    ...
                                    if (ok) {
                                        ...
                                        return {alpha_ref, it, false, dx_ref,
                                                ds_ref};
                                    }
                                }
                            }
            ...
            const double bt = (watchdog > cfg_.ls_watchdog_max / 2)
                                  ? (cfg_.ls_backtrack * 0.5)
                                  : cfg_.ls_backtrack;
            alpha *= bt;
            ++it;
            if (theta_t < theta0)
   ...
   private:
    ...
    bool eval_base_(ModelC* model, const dvec& x) const {
        ...
        constexpr uint32_t BASE_MASK =
            EV_F | EV_G | EV_cE | EV_cI | EV_JE | EV_JI;
        if (!model->eval_all_mask(x, BASE_MASK)) return false;
#else
        try {
            static const std::vector<std::string> comps{"f",  "g",  "cE",
                                                        "cI", "JE", "JI"};
    ...
    bool soc_least_squares_(const spmat* JE, const spmat* JI, const dvec* rE,
                            const dvec* rI, dvec& dx_cor) const {
        const bool hasE =
            JE && rE && JE->rows() == rE->size() && JE->rows() > 0;
        const bool hasI =
            JI && rI && JI->rows() == rI->size() && JI->rows() > 0;
