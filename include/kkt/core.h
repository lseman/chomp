...
public:
    ...
    factor_and_solve(ModelC *model_in, const spmat &W, const std::optional<spmat> &Gopt, const dvec &r1,
                     const std::optional<dvec> &r2opt, const KKTConfig &cfg, std::optional<double> /*regularizer*/,
...
private:
    ...
    dvec solve_schur_iterative_with_delta2(const spmat &G, const spmat &Gt, const std::function<dvec(const dvec &)> &Ks,
                                           const dvec &rhs_s, const spmat &K, const KKTConfig &cfg,
                                           bool use_prec) const {
        const int m = G.rows();
...
public:
    ...
    // Optional knobs (could be pulled from KKTConfig)
    struct Options {
...
public:
    ...
    factor_and_solve(ModelC *model_in, const spmat &W, const std::optional<spmat> &Gopt,
                     ...
                     const KKTConfig &cfg, std::optional<double> regularizer,
