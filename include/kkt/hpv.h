...
// - By default uses a robust single-RHS CG (cg_single) that calls only mv
// (HVP).
// - If KKT_USE_BLOCK_CG is defined, multi-RHS block CG (block_cg) is used where
// helpful.
//
...
#include "core.h"  // dvec, dmat, spmat, KKTConfig, cg(), LinOp, SSORPrecond, helpers like diag_GtG, schur_diag_hat, build_S_hat
...
inline std::pair<dvec, CGInfoBlock> cg_single(
    const BlockLinOp& A, const dvec& b, const ScalarJacobi& Minv, double tol,
    int maxit, const std::optional<dvec>& x0 = std::nullopt) {
    CGInfoBlock info;
...
inline std::pair<dmat, CGInfoBlock> block_cg(
    const BlockLinOp& A, const dmat& B, const BlockJacobi& Minv, double tol,
    int maxit, const std::optional<dmat>& X0 = std::nullopt,
    bool /*use_simd*/ = true) {
    CGInfoBlock info;
    ...
    auto max_rel = [&](const std::vector<double>& rn,
    ...
    if (rnorm.empty() || info.max_rel_resid <= tol) {
        info.converged = true;
        return {X, info};
    }
    ...
    Z.resize(n, kk_init);
    Znew.resize(n, kk_init);
    P.resize(n, kk_init);
    AP.resize(n, kk_init);
    RtZ.resize(kk_init, kk_init);
    RtZ_old.resize(kk_init, kk_init);
    RtZ_new.resize(kk_init, kk_init);
    PtAP.resize(kk_init, kk_init);
    alpha.resize(kk_init, kk_init);
    beta.resize(kk_init, kk_init);
    ...
    for (int it = 0; it < maxit; ++it) {
        ...
        keep_idx.reserve(rnorm.size());
        std::vector<double> b2, r2;
        b2.reserve(rnorm.size());
        r2.reserve(rnorm.size());
        for (int j = 0; j < (int)rnorm.size(); ++j) {
            const double denom = (bnorm[j] > 0.0) ? bnorm[j] : 1.0;
            if (rnorm[j] / denom > tol) {
                keep_idx.push_back(j);
                b2.push_back(bnorm[j]);
                r2.push_back(rnorm[j]);
            } else
                need_compact = true;
        }
        if (need_compact &&
            (it % comp_freq == 0 || (int)keep_idx.size() < (int)rnorm.size())) {
...
inline std::pair<dmat, CGInfoBlock> block_cg_k1(
    const BlockLinOp& A, const dmat& B, const BlockJacobi& Minv, double tol,
    int maxit, const std::optional<dmat>& X0 = std::nullopt,
    bool use_simd = true) {
...
class SchurHVPKKT {
   public:
    ...
    // Single RHS solve
    static std::tuple<dvec, dvec, SolveInfo> solve_once(
        const WOp& W, const std::optional<spmat>& Gopt, const dvec& r1,
        const dvec& r2, double delta, double gamma, const KKTConfig& cfg,
        bool assemble_schur_if_m_small = true, bool use_prec = true,
        const std::optional<dvec>& Kdiag_hint = std::nullopt) {
        const int n = static_cast<int>(r1.size());
        const int m =
            (Gopt && Gopt->rows() > 0) ? static_cast<int>(Gopt->rows()) : 0;

        // Prepare primals (enables lanes re-use)
        W.prepare();

        // Build K and operator
        KOp Kop{n, &W, delta, gamma,
                (Gopt && Gopt->rows() > 0) ? &(*Gopt) : nullptr};
        ...
        if (m == 0) {
            ...
            B.col(0) = r1;
            auto [Xsol, info0] =
                block_cg_k1(A, B, Minv_block, final_tol, final_it, std::nullopt,
                            cfg.use_simd);
            (void)info0;
            return {Xsol.col(0), dvec(), sinfo};
#else
            auto [dx, info0] = cg_single(A, r1, Minv_scalar, final_tol,
                                         final_it, std::nullopt);
            (void)info0;
        ...
        dmat B1(n, 1);
        B1.col(0) = svec;
        auto [Kinv_s_mat, info_s] =
            block_cg_k1(A, B1, Minv_block, assemble_tol, assemble_it,
                        std::nullopt, cfg.use_simd);
        (void)info_s;
        const dvec Kinvsvec = Kinv_s_mat.col(0);
#else
        auto [Kinvsvec, info_s] = cg_single(A, svec, Minv_scalar, assemble_tol,
                                            assemble_it, std::nullopt);
        (void)info_s;
#endif
        ...
        if (small_m) {
            ...
            B = Kop.G->transpose();
            auto [Z, infoZ] =
                block_cg_k1(A, B, Minv_block, assemble_tol, assemble_it,
                            std::nullopt, cfg.use_simd);
            (void)infoZ;
            dmat S = (*Kop.G) * Z;  // (m×m)
#else
            ...
            for (int j = 0; j < m; ++j) {
                ...
                auto [zj, infoZ] = cg_single(A, rhs, Minv_scalar, assemble_tol,
                                             assemble_it, std::nullopt);
                (void)infoZ;
                Z.col(j).noalias() = zj;
            }
...
#ifdef KKT_USE_BLOCK_CG
                           dmat BY(n, 1);
                           BY.col(0) = Kop.G->transpose() * y;
                           auto [Zy, infoY] = block_cg_k1(
                               A, BY, Minv_block, assemble_tol, assemble_it,
                               std::nullopt, cfg.use_simd);
                           (void)infoY;
            ...
            if (use_prec) {
                if (cfg.prec_type == "jacobi") {
                    SchurJacobiMinv =
                        schur_diag_hat(*Kop.G, Kdiag_inv).cwiseInverse();
            ...
            auto cg_res =
                cg(S_op, rhs_s, SchurJacobiMinv, cfg.cg_tol, cfg.cg_maxit,
                   std::nullopt, SchurSSOR, cfg.use_simd);
        ...
        dmat Bf(n, 1);
        Bf.col(0) = rhs_final;
        auto [Xfin, info_fin] = block_cg_k1(A, Bf, Minv_block, final_tol,
                                            final_it, X0, cfg.use_simd);
        (void)info_fin;
        return {Xfin.col(0), dy, sinfo};
#else
        auto [dx, info_fin] = cg_single(A, rhs_final, ScalarJacobi{Kdiag_inv},
                                        final_tol, final_it, Kinvsvec);
        (void)info_fin;
        return {dx, dy, sinfo};
#endif
    }
    ...
    class Reusable : public KKTReusable {
       public:
        Reusable(WOp W_, std::optional<spmat> G_, double delta_, double gamma_,
                 KKTConfig cfg_)
            : W(std::move(W_)),
              G(std::move(G_)),
              delta(delta_),
        ...
        std::pair<dvec, dvec> solve(const dvec& r1,
            ...
            if (!r2)
                throw std::invalid_argument("SchurHVPKKT::Reusable needs r2");
            KKTConfig local = cfg;
            local.cg_tol = cg_tol;
            local.cg_maxit = std::max(1, cg_maxit);

            auto [dx, dy, _] = SchurHVPKKT::solve_once(
                W, G, r1, *r2, delta, gamma, local, /*assemble small*/ true,
