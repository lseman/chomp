   ...
   public:
    ...
    const DegeneracySignals& begin_pricing(
        ...
        if (cycling_len_ >= 3 ||
            ...
            if (cond_est_ > 1e9) {
                signals_.preferred_strategy = PricingStrategy::DUAL_STEEPEST;
            } else {
   ...
   public:
    ...
    LeavingChoice choose_leaving(const BasisLike& B, const Eigen::VectorXd& yB,
        ...
        for (int i = 0; i < yB.size(); ++i) {
            ...
            e_i(i) = 1.0;
            Eigen::VectorXd psi_i = B.solve_BT(e_i);
            const double weight = std::max(1.0, psi_i.squaredNorm());
            const double infeas = -yB(i);
            const double score = (infeas * infeas) / weight;
            if (score > best_score) {
                best_score = score;
                best.row = i;
                best.dual_row = std::move(psi_i);
                best.weight = weight;
            }
        }
        return best;
...
class AdaptivePricer {
   public:
    ...
    static constexpr int kNumStrategies = 5;
    ...
    std::optional<int> choose_entering(const Eigen::VectorXd& rN,
        ...
        switch (current_strategy_) {
            ...
            case DUAL_STEEPEST:
                return dual_steepest_pricer_.choose_entering(rN, N, tol);
            case MOST_NEGATIVE:
    ...
    void update_after_pivot(int leaving_rel, int entering_abs, int old_abs,
                            ...
                            const Eigen::VectorXd* dual_row = nullptr) {
        if (dual_row != nullptr) {
            dual_steepest_pricer_.update_after_pivot(
                leaving_rel, entering_abs, old_abs, pivot_column, alpha, A, N,
                dual_row, true);
        } else {
            steepest_pricer_.update_after_pivot(leaving_rel, entering_abs,
                                                old_abs, pivot_column, alpha, A,
                                                N, true);
