...
struct EGraph {
    ...
    void canonicalize_kids(ENode& n) {
        for (auto& k : n.kids) k.id = find(k.id);
    }
    ...
    void unite(int a, int b) {
        ...
        if (classes[b].has_const) classes[a].cval = classes[b].cval;
        classes[a].nonzero |= classes[b].nonzero;
        classes[a].positive |= classes[b].positive;
        // append nodes
        for (int n : classes[b].nodes) classes[a].nodes.push_back(n);
    ...
    static size_t hash_node(const ENode& n) {
        ...
        if (n.is_symbol) h = mix(h, std::hash<const void*>{}(n.sym_ptr));
        for (auto k : n.kids) h = mix(h, std::hash<int>{}(k.id));
        return h;
    }
