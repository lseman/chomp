...
static ADNodePtr extract_rec(int eclass, const EGraph& EG, ECostModel& cm,
    ...
    if (n.is_symbol) {
        ...
        // Find or recreate a shared_ptr wrapper (we assume your graph retains
        // ownership) Fast path: your ADGraph keeps stable ADNodePtr; we reuse
        // via id map if needed. For now, we create a lightweight aliasing
        // shared_ptr (no deleter).
        ADNodePtr sptr(orig,
                       [](ADNode*) {});  // aliasing, no ownership transfer
        memo[rep] = sptr;
        return sptr;
    }
