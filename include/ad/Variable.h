...
class Variable : public std::enable_shared_from_this<Variable> {
   public:
    explicit Variable(const std::string& name, double value = 0.0,
    ...
    Variable& operator=(const Variable& other) {
        ...
        // NOTE: base part (enable_shared_from_this) cannot be reassigned; this
        // is fine.
        name_ = other.name_;
        value_ = other.value_;
        lb_ = other.lb_;
    ...
    static int getCurrentOrder() {
        return nextOrder_.load(std::memory_order_relaxed);
    }
    static void resetOrder(int start = 0) {
        nextOrder_.store(start, std::memory_order_relaxed);
    }
    ...
    ExpressionPtr operator*(double lhs) const;
